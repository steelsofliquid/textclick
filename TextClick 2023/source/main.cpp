...
BOOL GetFileName(HWND hwnd, LPSTR pszFileName, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "TextClick Documents (7 or later) (*.txcl)\0*.txcl\0Standard Text Files (*.txt)\0*.txt\0\0";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpszCmdParam, int nCmdShow) {
	...
	g_hMainWindow = CreateWindowEx(WS_EX_APPWINDOW,g_szAppName,"TextClick 2023",WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) {
	switch(Message) {
		case WM_CREATE: {
			...
			SendMessage(g_hStatusBar, SB_SETTEXT, 2, (LPARAM)"Demo Toolbar");
